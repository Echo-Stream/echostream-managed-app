...
async def initialize_app() -> None:
    ...
    async with Client(
        ...
        app_result = await client.execute(
            query,
            variable_values={"tenant": environ["TENANT"], "name": environ["APP_NAME"]},
        )
    if app_result["SearchApps"]["lastEvaluatedKey"]:
        raise Exception(f'More than one configuration found for {environ["APP_NAME"]}')
    if not app_result["SearchApps"]["items"]:
        raise Exception(f'No configuration found for {environ["APP_NAME"]}')
    await asyncio.gather(
        *[
    ...
    try:
        if not node_config:
            async with Client(
                ...
                node_result = await client.execute(
                    query, variable_values={"tenant": environ["TENANT"], "name": node}
                )
        ...
        if image_ref.repository["domain"] and ECR_PATTERN.match(
            ...
            if username or password:
                raise Exception(
                    "Username and password are mutually exclusive to using an ecr repo."
                )
            ...
            username, password = (
                b64decode(
                    authorization_data["authorizationData"][0]["authorizationToken"]
                )
                .decode()
                .split(":")
            )
        ...
        if (username, password) and None in (username, password):
            raise Exception(
                "You must provide both username AND password for private registries or neither for public registries"
            )
...
async def run() -> None:
    ...
    async with Client(
        ...
        async for notification in client.subscribe(
            subscription,
            variable_values={"tenant": environ["TENANT"], "app": environ["APP_NAME"]},
        ):
